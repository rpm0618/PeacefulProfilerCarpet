 ...
 * @see HashCommon
 */
public class CustomHashMap<V> extends AbstractLong2ObjectMap<V> implements java.io.Serializable, Cloneable, Hash {
...
//    protected transient volatile V[] value;
    /** The mask for wrapping a position counter. */
    public transient int mask;
//    public volatile transient int mask;
    ...
    public CustomHashMap(final int expected, final float f) {
     ...
     */
    public CustomHashMap(final int expected) {
     ...
     *            the load factor.
     */
    public CustomHashMap(final Map<? extends Long, ? extends V> m, final float f) {
        this(m.size(), f);
        putAll(m);
    }
     ...
     *            a {@link Map} to be copied into the new hash map.
     */
    public CustomHashMap(final Map<? extends Long, ? extends V> m) {
     ...
     *            the load factor.
     */
    public CustomHashMap(final Long2ObjectMap<V> m, final float f) {
        this(m.size(), f);
        putAll(m);
    }
     ...
     *            a type-specific map to be copied into the new hash map.
     */
    public CustomHashMap(final Long2ObjectMap<V> m) {
        this(m, DEFAULT_LOAD_FACTOR);
    }
     ...
     */
    public CustomHashMap(final long[] k, final V[] v, final float f) {
        this(k.length, f);
     ...
     *             if <code>k</code> and <code>v</code> have different lengths.
     */
    public CustomHashMap(final long[] k, final V[] v) {
    ...
    private int insert(final long k, final V v) {
        ...
        if (size++ >= maxFill) {
            ...
            if (k == ChunkPos.asLong(ProfileConfig.REHASH_CHUNK.x, ProfileConfig.REHASH_CHUNK.z)) {
                pre = BlockBeacon.GET_BLOCK_COUNT.get();
            }
            // end rpm0618
            ...
            // rpm0618
            if (k == ChunkPos.asLong(ProfileConfig.REHASH_CHUNK.x, ProfileConfig.REHASH_CHUNK.z)) {
                post = BlockBeacon.GET_BLOCK_COUNT.get();
                System.out.println("Correct chunk triggered rehash: " + ChunkPos.fromLong(k) + " pre=" + pre + " post=" + post);
            } else {
                System.out.println("Wrong chunk triggered rehash: " + ChunkPos.fromLong(k));
            }
            // end rpm0618
        }
    ...
    public boolean containsValue(final Object v) {
        ...
        for (int i = n; i-- != 0;)
            if (!((key[i]) == (0)) && ((value[i]) == null ? (v) == null : (value[i]).equals(v))) return true;
        return false;
    }
    ...
    private class MapIterator {
         ...
         * key.
         */
        boolean mustReturnNullKey = CustomHashMap.this.containsNullKey;
        /**
        ...
        public int nextEntry() {
            ...
            final long key[] = CustomHashMap.this.key;
            for (;;) {
                if (--pos < 0) {
        ...
        public void remove() {
            ...
            if (last == n) {
                ...
                CustomHashMap.this.remove(wrapped.getLong(-pos - 1));
    ...
    private class EntryIterator extends CustomHashMap.MapIterator implements ObjectIterator<Long2ObjectMap.Entry<V>> {
        private CustomHashMap.MapEntry entry;

        public Long2ObjectMap.Entry<V> next() {
            return entry = new CustomHashMap.MapEntry(nextEntry());
        }
    ...
    private class FastEntryIterator extends CustomHashMap.MapIterator implements ObjectIterator<Long2ObjectMap.Entry<V>> {
        private final CustomHashMap.MapEntry entry = new CustomHashMap.MapEntry();

        public CustomHashMap.MapEntry next() {
    ...
    private final class MapEntrySet extends AbstractObjectSet<Long2ObjectMap.Entry<V>> implements FastEntrySet<V> {
        public ObjectIterator<Long2ObjectMap.Entry<V>> iterator() {
            return new CustomHashMap.EntryIterator();
        }

        public ObjectIterator<Long2ObjectMap.Entry<V>> fastIterator() {
            return new CustomHashMap.FastEntryIterator();
        }
        ...
        public boolean contains(final Object o) {
            ...
            if (((k) == (0))) return CustomHashMap.this.containsNullKey && ((value[n]) == null ? (v) == null : (value[n]).equals(v));
            long curr;
            final long[] key = CustomHashMap.this.key;
            int pos;
            // The starting point.
            if (((curr = key[pos = (int) it.unimi.dsi.fastutil.HashCommon.mix((k)) & mask]) == (0))) return false;
            if (((k) == (curr))) return ((value[pos]) == null ? (v) == null : (value[pos]).equals(v));
            // There's always an unused entry.
            while (true) {
                if (((curr = key[pos = (pos + 1) & mask]) == (0))) return false;
                if (((k) == (curr))) return ((value[pos]) == null ? (v) == null : (value[pos]).equals(v));
    ...
    public FastEntrySet<V> long2ObjectEntrySet() {
        if (entries == null) entries = new CustomHashMap.MapEntrySet();
        return entries;
    }
     ...
     */
    private final class KeyIterator extends CustomHashMap.MapIterator implements LongIterator {
        public KeyIterator() {
            super();
        }
    ...
    private final class KeySet extends AbstractLongSet {
        public LongIterator iterator() {
            return new CustomHashMap.KeyIterator();
        }
    ...
    public LongSet keySet() {
        if (keys == null) keys = new CustomHashMap.KeySet();
        return keys;
    }
     ...
     */
    private final class ValueIterator extends CustomHashMap.MapIterator implements ObjectIterator<V> {
    ...
    public ObjectCollection<V> values() {
        if (values == null) values = new AbstractObjectCollection<V>() {
            public ObjectIterator<V> iterator() {
                return new CustomHashMap.ValueIterator();
            }
     ...
     */
    @SuppressWarnings("unchecked")
    public CustomHashMap<V> clone() {
        CustomHashMap<V> c;
        try {
            c = (CustomHashMap<V>) super.clone();
    ...
    private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
        final long key[] = this.key;
        final V value[] = this.value;
        final CustomHashMap.MapIterator i = new CustomHashMap.MapIterator();
